# Задание Нарежьте и подключите новые шаблоны к проекту «Анфиса для друзей»,
# развёрнутому на вашем компьютере. Порядок выполнения задания:
#
# Скачайте и распакуйте набор HTML-шаблонов. В HTML-файлах комментариями
# отмечены границы блоков, по которым нужно разделить файлы на шаблоны.
#
# Перенесите код в соответствующие файлы — в base.html, в расширяющие файлы
# и в файлы, подключаемые через {% include %}.
#
# Чтобы отобразить несколько
# одинаковых элементов на странице, пройдите циклом {% for %} по коллекции,
# которая хранит эти элементы.
#
# Если какой-то фрагмент HTML-кода встречается
# больше двух раз, выносите этот фрагмент в отдельный файл и включайте его в
# основной код через {% include %}.
#
# При необходимости применяйте тег {% with
# %}. В header будет удобно использовать {% with
# request.resolver_match.view_name as ... %} для упрощения работы со
# ссылками.
#
# Статические файлы подгружаются с помощью {% static % },
# а для ссылок используются {% url % }.
#
# Запустите проект в dev-режиме, убедитесь, что всё работает. Для полной
# уверенности запустите тесты. В директории Dev/anfisa1sprint/ выполните
# команду pytest. Виртуальное окружение должно быть активно. Если всё
# сделано правильно, в консоли появится такой вывод: Скрин
import difflib
import inspect
import os
import re
